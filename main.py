...
def draw_stars(canv, w, h, count=140, seed=0):
    ...
    for _ in range(5):
        x = rng2.uniform(12*mm, w-12*mm); y = rng2.uniform(12*mm, h-12*mm)
...
def draw_mandala(canv, cx, cy, radius=28*mm, alpha=0.25):
    ...
    for r in [radius, radius*0.7, radius*0.45, radius*0.2]:
        canv.setLineWidth(0.4); canv.circle(cx, cy, r, fill=0, stroke=1)
    canv.setLineWidth(0.3)
    for i in range(8):
        angle = i*math.pi/4
    ...
    for i in range(8):
        ...
        d=2.5; p=canv.beginPath()
        p.moveTo(px,py+d); p.lineTo(px+d,py)
        p.lineTo(px,py-d); p.lineTo(px-d,py); p.close()
...
def cover_bg(canv, doc):
    canv.saveState()
    canv.setFillColor(DEEP_SPACE); canv.rect(0,0,W,H,fill=1,stroke=0)
    for i in range(20):
        f=i/20
...
def inner_bg(canv, doc):
    canv.saveState()
    canv.setFillColor(MIDNIGHT); canv.rect(0,0,W,H,fill=1,stroke=0)
    for i in range(10):
        f=i/10
    ...
    canv.setStrokeColor(GOLD_DIM); canv.setLineWidth(0.6)
    ...
    canv.setStrokeColor(GOLD_DIM); canv.setLineWidth(0.4)
...
class CoverPage(Flowable):
    def __init__(self, name, birth_date, birth_time, birth_place):
        super().__init__()
        self.name=name; self.birth_date=birth_date
        self.birth_time=birth_time; self.birth_place=birth_place

    def wrap(self, aw, ah):
        self.width=aw; self.height=ah; return aw, ah

    def draw(self):
        c=self.canv; cx=self.width/2; h=self.height
        c.saveState()
        ...
        c.setStrokeColor(GOLD_DIM); c.setLineWidth(0.5)
        ...
        c.setFillColor(GOLD); c.setFont("Helvetica", 13)
        ...
        c.setFillColor(GOLD_BRIGHT); c.setFont("Helvetica-Bold", 48)
        c.drawCentredString(cx, h-74*mm, "Dharma Code\u2122")
        c.setStrokeColor(GOLD); c.setLineWidth(1.5)
        c.line(cx-58*mm, h-78*mm, cx+58*mm, h-78*mm)
        c.setStrokeColor(colors.Color(0.83,0.69,0.35,alpha=0.20)); c.setLineWidth(5)
        c.line(cx-52*mm, h-78*mm, cx+52*mm, h-78*mm)
        c.setFillColor(STAR_WHITE); c.setFont("Helvetica", 19)
        ...
        c.setStrokeColor(colors.Color(0.45,0.22,0.75,alpha=0.35)); c.setLineWidth(0.4)
        c.line(cx-58*mm, h/2+6*mm, cx-20*mm, h/2+6*mm)
        c.line(cx+20*mm, h/2+6*mm, cx+58*mm, h/2+6*mm)
        c.setFillColor(GOLD); c.setFont("Helvetica", 14)
        c.drawCentredString(cx, h/2+2*mm, "\u2736  \u2736  \u2736")
        c.setFillColor(STAR_WHITE); c.setFont("Helvetica-Oblique", 15)
...
class CosmicDivider(Flowable):
    def __init__(self, before=6*mm, after=6*mm):
        super().__init__(); self._b=before; self._a=after

    def wrap(self, aw, ah):
        self.width=aw; self.height=8*mm+self._b+self._a; return self.width, self.height

    def draw(self):
        c=self.canv; cx=self.width/2; y=self._a+4*mm
        c.saveState()
        c.setStrokeColor(colors.Color(0.50,0.35,0.10,alpha=0.50)); c.setLineWidth(0.5)
        c.line(0, y, cx-18, y); c.line(cx+18, y, self.width, y)
...
class SectionHeader(Flowable):
    def __init__(self, number, label):
        super().__init__(); self._num=number; self._label=label

    def wrap(self, aw, ah):
        self.width=aw; self.height=20*mm; return self.width, self.height

    def draw(self):
        c=self.canv; c.saveState(); y_base=8*mm
        c.setFillColor(GOLD_BRIGHT); c.setFont("Helvetica-Bold", 32)
        c.drawString(0, y_base, str(self._num))
        c.setFillColor(LAVENDER); c.setFont("Helvetica", 9)
...
class PullQuote(Flowable):
    ...
    def draw(self):
        ...
        c.setFillColor(GOLD_BRIGHT); c.setFont("Helvetica-BoldOblique", 13)
...
class CTAStep(Flowable):
    ...
    def draw(self):
        ...
        c.setStrokeColor(colors.Color(0.45,0.25,0.75,alpha=0.50)); c.setLineWidth(0.6)
        c.roundRect(0,0,self.width,h,5,fill=0,stroke=1)
        c.setFillColor(GOLD); c.circle(8*mm, h-7*mm, 5*mm, fill=1, stroke=0)
        c.setFillColor(DEEP_SPACE); c.setFont("Helvetica-Bold", 9)
        c.drawCentredString(8*mm, h-8.5*mm, str(self._num).zfill(2))
        c.setFillColor(GOLD_BRIGHT); c.setFont("Helvetica-Bold", 12)
        c.drawString(17*mm, h-7.5*mm, self._title)
        c.setFillColor(CREAM); c.setFont("Helvetica", 10)
        max_w = self.width-19*mm
        words=self._body.split(); line=""; y=h-14*mm
        for word in words:
            test=(line+" "+word).strip()
            if c.stringWidth(test,"Helvetica",10) <= max_w:
                line=test
            else:
                c.drawString(17*mm, y, line); y-=4.8*mm; line=word
...
class StatsBar(Flowable):
    def __init__(self, stats):
        super().__init__(); self._stats=stats
    ...
    def draw(self):
        ...
        n=len(self._stats); col_w=self.width/n
        for i,(num,label) in enumerate(self._stats):
            cx=col_w*i+col_w/2
            c.setFillColor(GOLD_BRIGHT); c.setFont("Helvetica-Bold", 14)
            c.drawCentredString(cx, 11.5*mm, num)
            c.setFillColor(LAVENDER); c.setFont("Helvetica", 7)
            c.drawCentredString(cx, 6*mm, spaced(label))
            if i < n-1:
                c.setStrokeColor(colors.Color(0.5,0.3,0.8,alpha=0.3)); c.setLineWidth(0.4)
...
def parse_reading(text):
    ...
    current="opening"; buffer=[]
    for line in lines:
        stripped=line.strip(); matched=False
        for sec, keywords in markers.items():
            for kw in keywords:
                if (stripped.lower()==kw.lower() or
                        (stripped.lower().startswith(kw.lower()) and len(stripped)<50)):
                    if buffer: sections[current]="\n".join(buffer).strip()
                    current=sec; buffer=[]; matched=True; break
            if matched: break
        if not matched: buffer.append(line)
    if buffer: sections[current]="\n".join(buffer).strip()
    return sections
...
def generate_file():
    ...
    for field in ["name","birth_date","birth_place","reading_text"]:
        if field not in data: return jsonify({"error":f"Missing field: {field}"}), 400
    name=data["name"]; birth_date=data.get("birth_date","")
    birth_time=data.get("birth_time","unknown"); birth_place=data.get("birth_place","")
