...
def parse_reading(text):
    ...
    """
    # Strip internal calculation block if present
    if "INTERNAL CALCULATION" in text:
        parts = text.split("INTERNAL CALCULATION")
        # Find where the actual reading starts - look for "Hi " or the opening
        pre = parts[0]
        # Get everything after the calculation block by finding next "Hi " 
        rest = "INTERNAL CALCULATION".join(parts[1:])
        # Find end of calculation block - it ends when the reading starts
        # Look for the pattern that ends the calc and starts the reading
        reading_start = rest.find("\nHi ")
        if reading_start == -1:
            reading_start = rest.find("\n\nHi ")
        if reading_start != -1:
            text = pre + rest[reading_start:]
        else:
            text = pre
    ...
    markers = {
        "section1": ["You Are an Awakened Guide", "YOU ARE AN AWAKENED GUIDE", "awakened guide"],
        "section2": ["Your Gifts", "YOUR GIFTS"],
        "section3": ["Your Shadow Code", "YOUR SHADOW CODE", "shadow code"],
        "section4": ["What You Need", "WHAT YOU NEED"],
        "cta":      ["Your Next Step", "YOUR NEXT STEP"],
    }
    ...
    for line in lines:
        ...
        for sec, keywords in markers.items():
            for kw in keywords:
                if kw.lower() in stripped.lower() and len(stripped) < 80:
                    if buffer:
                        sections[current] = "\n".join(buffer).strip()
                    current = sec
                    buffer  = []
